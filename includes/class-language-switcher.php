...
class TRP_Language_Switcher{
	...
	private function get_current_language( $trp ){
        ...
        $browserLang = str_replace('-', '_', substr($_SERVER['HTTP_ACCEPT_LANGUAGE'], 0, 3));
        session_start();
        $currentLangValidation = empty($_SESSION['current_lang']) && $_SESSION['current_lang'] !== 'fr_CA';
        if (strtolower($browserLang) === 'fr_' && empty($language_from_url) && $currentLangValidation) {
            $needed_language = 'fr_CA';
            $_SESSION['current_lang'] = $needed_language;
        }
